...
#
# Prerequisites:
#   Debian/Ubuntu: apt install cmake bison libssl-dev libncurses-dev \
#                              pkg-config build-essential
#   RHEL/Fedora:   dnf install cmake bison openssl-devel ncurses-devel \
#                              pkg-config gcc-c++
# =============================================================================
...
if ! command -v cmake &>/dev/null; then
  ...
  error "  Debian/Ubuntu: apt install cmake"
  error "  RHEL/Fedora:   dnf install cmake"
  exit 1
fi
...
if ! command -v bison &>/dev/null; then
  ...
  warn "  Debian/Ubuntu: apt install bison"
  warn "  RHEL/Fedora:   dnf install bison"
else
...
if [[ -z "$OPENSSL_DIR" ]]; then
  for candidate in /usr /usr/local; do
    if [[ -f "${candidate}/lib/libcrypto.a"    || \
...
if command -v pkg-config &>/dev/null && pkg-config --exists ncurses 2>/dev/null; then
  CURSES_DIR="$(pkg-config --variable=prefix ncurses)"
fi

if [[ -z "$CURSES_DIR" ]]; then
  for candidate in /usr /usr/local; do
    if [[ -f "${candidate}/lib/libncurses.a"    || \
          -f "${candidate}/lib/libncurses.so"   || \
          -f "${candidate}/lib64/libncurses.so" ]]; then
...
if [[ -n "$CURSES_DIR" ]]; then
  ...
  warn "  Debian/Ubuntu: apt install libncurses-dev"
  warn "  RHEL/Fedora:   dnf install ncurses-devel"
fi
...
for ARCHIVE in "${ARCHIVES[@]}"; do
  ...
  if [[ -n "$OPENSSL_DIR" ]]; then
    CMAKE_FLAGS+=(-DWITH_SSL="${OPENSSL_DIR}")
  else
